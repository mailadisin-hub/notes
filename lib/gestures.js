...
/**
 * A scribble: back and forth several times over a small area, the way anyone
 * crosses something out.
 *
 * The test is deliberately hard to pass. Writing is full of sharp turns - m, w,
 * z - so a stroke only counts when it doubles back at least three times AND
 * covers three times its own size in doing it. Handwriting does the first; a
 * cursive w doubles back three times but only covers about two and a half of
 * itself, and almost nothing but a crossing-out does both.
 */
export function looksLikeScribble(pts) {
  const p = xy(pts);
  if (p.length < 12) return false;
  const box = bounds(p);
  if (box.diag < 12) return false;

  const length = pathLength(p);
  if (length < box.diag * 3) return false;
