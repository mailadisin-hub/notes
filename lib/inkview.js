 ...
 *   - a mouse writes, the wheel zooms, shift+wheel scrolls and the middle
 *     button drags the board about, for the laptop.
...
import { looksLikeScribble, recogniseShape, snapToRuler, projectOnRuler } from './gestures.js';
...
const EDGE_PAD = 20;
const HOLD_MS = 450;
/* Hold the pen still at the end of a shape and it is redrawn neatly. Longer
   than a pause for thought, short enough not to feel like waiting. */
const SHAPE_HOLD_MS = 550;
const SHAPE_STILL_PX = 6;
/* Carry on drawing after it has snapped and the snap is undone, so a shape
   that was not wanted costs nothing. */
const SHAPE_ESCAPE_PX = 26;
/* How far from the ruler's edge the pen still runs along it. */
const RULER_REACH = 34;
const TAP_MS = 320;
const TAP_SLOP_PX = 8;
const MIN_IMAGE = 32;
...
export function inkView(opts) {
  ...
  /* A whiteboard is one huge fixed sheet rather than a page that grows: there
     is already room in every direction, so nothing has to be added. */
  const boardPage = () => (pages.length === 1 && pages[0].board ? pages[0] : null);

  function growPage() {
    if (!opts.grow || boardPage()) return;
    const page = pages[0];
    let bottom = 0;
  ...
  function measure() {
    ...
    /* On a board, fitting the whole sheet would leave writing far too small to
       read, so a fit is one page-width of it: handwriting comes out the size it
       is in an ordinary note, and the board opens in the middle. */
    const board = boardPage();
    const nextFit = Math.max(0.05, (W - pad * 2) / (board ? board.unit || docW : docW));
    if (!ready) {
      cam.scale = Math.min(nextFit, opts.maxFitScale || Infinity);
      cam.x = board ? board.width / 2 - W / 2 / cam.scale : 0;
      cam.y = board ? board.height / 2 - H / 2 / cam.scale : -topPad / cam.scale;
      ready = true;
  ...
  function clampCam() {
    /* A board is meant to be stood back from, so it zooms out much further than
       a page, where there would be nothing out there to see. */
    const minScale = fitScale * (boardPage() ? 0.08 : 0.5);
  ...
  let hover = null;
  let ghosts = [];
  /* The straight-edge, in page units: where its middle is, which way it runs,
     and how long it is. Null when it is put away. */
  let ruler = null;
  let rulerDrag = null;
  ...
  function renderLive() {
    ...
    /* Everyone else's writing, as it happens. These live on the same canvas as
       our own unfinished stroke and are replaced by the real thing the moment
       the server has it, so nothing is drawn twice. */
    for (const g of ghosts) {
      const i = g.page || 0;
      const page = pages[i];
      if (!page || !g.pts || g.pts.length < 3) continue;
      const tool = TOOLS[g.tool] || TOOLS.pen;
      lctx.setTransform(s, 0, 0, s, (lefts[i] - cam.x) * s, (tops[i] - cam.y) * s);
      lctx.globalAlpha = tool.alpha;
      lctx.fillStyle = resolveColour(g.colour, pageDark(page));
      lctx.fill(outline(g));
      lctx.globalAlpha = 1;
    }
  ...
  function beginStroke(e, erase) {
    ...
    if (erase) {
      ...
      /* A stroke that starts against the ruler runs along it for its whole
         length, the way a pencil does - rather than following the pen off the
         edge and leaving half a straight line. */
      if (ruler) {
        const on = snapToRuler(ruler, pt.x, pt.y, RULER_REACH / cam.scale);
        if (on) {
          active.onRuler = true;
          pt.x = on.x;
          pt.y = on.y;
  ...
  /* Redraws the stroke as the shape it was meant to be. The original points are
     kept so that drawing on again puts them back: the snap is a suggestion held
     open until the pen lifts, not a decision made for you. */
  function snapShape() {
    if (!active || !active.stroke || active.snapped) return;
    const found = recogniseShape(active.stroke.pts);
    if (!found) return;
    active.snapped = { raw: active.stroke.pts, at: { x: active.still.x, y: active.still.y } };
    active.stroke.pts = found.pts;
    active.tail = null;
    delete active.stroke._path;
    delete active.stroke._box;
    haptic('commit');
    renderLive();
  }

  function unsnap() {
    if (!active || !active.snapped) return;
    active.stroke.pts = active.snapped.raw;
    active.snapped = null;
    delete active.stroke._path;
    delete active.stroke._box;
  }

  /* Watches for the pen standing still, which is what asks for the shape. */
  function watchStill(e) {
    if (!active || !active.stroke || readOnly || active.onRuler) return;
    const still = active.still;
    if (still && Math.hypot(e.clientX - still.x, e.clientY - still.y) <= SHAPE_STILL_PX) return;
    if (still) clearTimeout(still.timer);
    active.still = {
      x: e.clientX,
      y: e.clientY,
      timer: setTimeout(snapShape, SHAPE_HOLD_MS),
    };
  }

  function dropStill() {
    if (active && active.still) {
      clearTimeout(active.still.timer);
      active.still = null;
    }
  }
  ...
  function addSample(e) {
    ...
    if (active.snapped) {
      // Moving well away from where it snapped means it was not what was wanted.
      if (Math.hypot(e.clientX - active.snapped.at.x, e.clientY - active.snapped.at.y) < SHAPE_ESCAPE_PX) return;
      unsnap();
    }
    watchStill(e);
    const pt = toPage(e.clientX, e.clientY, active.page);
    // Started against the ruler, so it follows the edge to the end of the line.
    if (active.onRuler && ruler) Object.assign(pt, projectOnRuler(ruler, pt.x, pt.y));
    ...
    active.sx = sx;
    active.sy = sy;
    // So a shared page can send the stroke while it is still being written.
    if (opts.onDrawing && !active.erase) opts.onDrawing(active.stroke, active.page);
  }
  ...
  /**
   * A scribble drawn over existing writing is a crossing-out, so it takes that
   * writing with it instead of being added. It only counts when it actually
   * covers something: a scribble in empty space is just a scribble.
   */
  function scribbledOut(stroke, pageIndex) {
    if (readOnly || !opts.scribbleErases || !opts.scribbleErases()) return false;
    if (!looksLikeScribble(stroke.pts)) return false;
    const strokes = pages[pageIndex].strokes;
    const reach = stroke.size / 2 + 2;
    const removed = [];
    for (let k = strokes.length - 1; k >= 0; k -= 1) {
      const other = strokes[k];
      for (let i = 0; i < stroke.pts.length; i += 3) {
        if (hits(other, stroke.pts[i], stroke.pts[i + 1], reach)) {
          removed.push({ stroke: other, index: k });
          break;
        }
      }
    }
    if (!removed.length) return false;
    for (const r of removed) strokes.splice(r.index, 1);
    record({ type: 'erase', page: pageIndex, removed });
    haptic('commit');
    // Said out loud, because ink disappearing on its own should never be a
    // surprise you have to work out - and undo puts it straight back.
    if (opts.onScribbleErase) opts.onScribbleErase(removed.length);
    renderBase();
    changed(pageIndex);
    return true;
  }
  ...
  const RULER_WIDTH = 78;       // page units, so it sits on the paper like an object
  const RULER_GRAB_END = 0.34;  // the share of each end that turns it instead

  /** Where a point falls on the ruler: its end, its body, or neither. */
  function rulerGrab(x, y) {
    ...
    if (away < -4 || away > RULER_WIDTH || Math.abs(along) > half) return null;
    return Math.abs(along) > half * (1 - RULER_GRAB_END) ? 'rotate' : 'move';
  }
  ...
  function drawRuler(ctx) {
    ...
    const half = ruler.length / 2;
    const w = RULER_WIDTH;
    const hair = 1 / cam.scale;   // a line that stays one screen pixel at any zoom

    const body = ctx.createLinearGradient(0, 0, 0, w);
    body.addColorStop(0, 'rgba(226,228,233,0.92)');
    body.addColorStop(0.12, 'rgba(198,201,208,0.88)');
    body.addColorStop(0.5, 'rgba(232,234,239,0.86)');
    body.addColorStop(0.88, 'rgba(176,180,188,0.88)');
    body.addColorStop(1, 'rgba(150,154,163,0.9)');
    ctx.fillStyle = body;
    ctx.beginPath();
    if (ctx.roundRect) ctx.roundRect(-half, 0, ruler.length, w, 5);
    else ctx.rect(-half, 0, ruler.length, w);
    ctx.fill();

    // The drawing edge, brighter than the rest so it is obvious which side works.
    ctx.fillStyle = 'rgba(255,255,255,0.85)';
    ctx.fillRect(-half, 0, ruler.length, hair);
    ...
    ctx.fillStyle = 'rgba(40,40,46,0.72)';
    for (let d = -Math.floor(half / 10) * 10; d <= half; d += 10) {
      const big = Math.round(d) % 50 === 0;
      ctx.fillRect(d, 0, hair, big ? 17 : 9);
    }

    // The ends are where it turns, so they are marked as something to hold.
    ctx.fillStyle = 'rgba(60,60,67,0.16)';
    const grip = half * RULER_GRAB_END;
    ctx.fillRect(-half, w - 13, grip, 13);
    ctx.fillRect(half - grip, w - 13, grip, 13);
  }
  ...
  function endStroke() {
    ...
    if (done.erase) {
      ...
      if (done.tail && !done.snapped) done.stroke.pts.push(...done.tail);
      if (!done.snapped && scribbledOut(done.stroke, done.page)) {
        if (opts.onDrawing) opts.onDrawing(null, done.page);
        renderLive();
        return;
      }
  ...
  const onDown = (e) => {
    ...
    /* The ruler is an object lying on the page: take hold of its middle to
       slide it, or either end to turn it. */
    if (ruler && !readOnly && !active && !rulerDrag && !touches.size) {
      const at = toPage(e.clientX, e.clientY);
      const grab = at ? rulerGrab(at.x, at.y) : null;
      if (grab) {
        ...
        rulerDrag = {
          ...
          y: at.y,
          angle: ruler.angle,
          from: Math.atan2(at.y - ruler.y, at.x - ruler.x),
        };
        haptic('select');
        return;
      }
    ...
    /* Locked for reading: a pen or mouse drag pans, exactly as a finger does.
       While writing, a left drag is the pen, so the middle button is how a
       mouse slides the board around - the wheel is busy zooming. */
    const pans = e.pointerType === 'touch'
      || (e.pointerType === 'mouse' && e.button === 1)
      || (readOnly && (e.pointerType === 'pen' || e.pointerType === 'mouse'));
    if (pans) {
      ...
      capture(el, e.pointerId);
      // Otherwise the middle button starts the browser's own autoscroll.
  ...
  const onMove = (e) => {
    ...
    if (rulerDrag && e.pointerId === rulerDrag.id) {
      const at = toPage(e.clientX, e.clientY, 0);
      if (rulerDrag.mode === 'move') {
        ruler.x += at.x - rulerDrag.x;
        ruler.y += at.y - rulerDrag.y;
        rulerDrag.x = at.x;
        rulerDrag.y = at.y;
      } else {
        const now = Math.atan2(at.y - ruler.y, at.x - ruler.x);
        ruler.angle = rulerDrag.angle + (now - rulerDrag.from);
      }
      schedule(LIVE);
      return;
    }
  ...
  const onUp = (e) => {
    ...
    if (rulerDrag && e.pointerId === rulerDrag.id) {
      rulerDrag = null;
      if (opts.onRuler) opts.onRuler({ ...ruler });
      return;
    }
    if (active && e.pointerId === active.id) {
      endStroke();
  ...
  const onWheel = (e) => {
    ...
    /* A wheel notch is reported in pixels, lines or pages depending on the
       mouse and the browser; in lines it is about 3 where pixels would be 100,
       so zooming would barely move without this. */
    const unit = e.deltaMode === 1 ? 16 : (e.deltaMode === 2 ? H : 1);
    const dx = e.deltaX * unit;
    const dy = e.deltaY * unit;

    /* The wheel zooms, which is what a board wants - there is no bottom of the
       page to scroll towards. Shift is the way to scroll instead, and a
       trackpad's sideways swipe still slides across. */
    if (e.shiftKey) {
      cam.x += (dx || dy) / cam.scale;
      if (dx) cam.y += dy / cam.scale;
    } else {
      const docX = (e.clientX - rect.left) / cam.scale + cam.x;
      const docY = (e.clientY - rect.top) / cam.scale + cam.y;
      cam.scale *= Math.exp(-dy * 0.0022);
      clampCam();
  ...
  return {
    ...
    /**
     * What other people are writing this second: stroke-shaped objects that are
     * drawn over the page until the finished stroke arrives.
     */
    /** The straight-edge, or null when it is put away. */
    get ruler() { return ruler ? { ...ruler } : null; },
    /**
     * Puts the ruler away, or lays it across what is on screen. Passing a ruler
     * back restores one that was saved.
     */
    setRuler(next) {
      if (next === true) {
        const across = W / cam.scale;
        ruler = {
          x: cam.x + across / 2,
          y: cam.y + (H / cam.scale) / 2,
          angle: 0,
          length: Math.max(240, Math.min(1000, across * 0.75)),
        };
      } else {
        ruler = next ? { ...next } : null;
      }
      rulerDrag = null;
      schedule(LIVE);
      return ruler ? { ...ruler } : null;
    },
    setGhosts(list) {
    ...
    /**
     * Back to the writing, at normal size. On a board you can pan into empty
     * space until nothing is on screen and there is no edge to find your way
     * back by, so this is the way back.
     */
    recentre() {
      if (!ready) return;
      const page = pages[0];
      ...
      /* Show all of the writing, standing back far enough for the whole of it
         if it is spread out - but never closer than normal reading size. */
      const pad = 40;
      const width = box ? box[2] - box[0] + pad * 2 : 0;
      const height = box ? box[3] - box[1] + pad * 2 : 0;
      cam.scale = width > 0 && height > 0
        ? Math.min(fitScale, W / width, H / height)
        : fitScale;
      const cx = box ? (box[0] + box[2]) / 2 : page.width / 2;
      const cy = box ? (box[1] + box[3]) / 2 : page.height / 2;
      cam.x = cx - W / 2 / cam.scale;
      cam.y = cy - H / 2 / cam.scale;
      clampCam();
      renderBase();
    },
    /**
     * After the page itself changed shape - a note turning into a board, or
     * back - so the camera is placed afresh rather than left where a page that
     * no longer exists used to be.
     */
    reframe() {
      ready = false;
      measure();
